...
`WKWebView.customUserAgent` には、`InstagramWebView.mobileSafariUserAgent` 定数で集約しているモバイル Safari の User-Agent 文字列を設定します。Instagram モバイル Web 版は UA を見てモバイル UI / 機能セットに分岐するため、`Mozilla/5.0 (iPhone; ...) AppleWebKit/... Version/<x>.0 Mobile/<build> Safari/...` というフォーマットを維持する必要があります。

- iOS バージョンを bump したいときは `InstagramWebView.swift` の `mobileSafariUserAgent` 定数 1 箇所だけを編集すれば良いように `static let` にしています（旧実装では `makeUIView` 内のマジック文字列だったため検索が必要でした）。
- `InstaDirectOnlyTests/InstagramWebViewURLPolicyTests.swift` には、空でないこと・`iPhone` / `Mobile/` / `Safari/` / `AppleWebKit/` / `Mozilla/5.0` プレフィクスを含むこと・制御文字を含まないことの最低限のフォーマット検査が含まれています。UA を編集したらこのテストが落ちないことを確認してください。
